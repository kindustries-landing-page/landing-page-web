    ...
    <Router>
      ...
      <Dialog open={isTestDriveOpen} onOpenChange={setTestDriveOpen}>
        <DialogContent className="max-w-[460px] bg-white/95 backdrop-blur-3xl border border-white rounded-[28px] p-10 shadow-[0_48px_100px_rgba(75,0,118,0.2)]">
          <DialogTitle className="text-[22px] font-extrabold text-[#4B0076] mb-1.5">Đặt lịch lái thử</DialogTitle>
          <p className="text-[14px] text-zinc-600 mb-6">Điền thông tin – chúng tôi xác nhận trong 30 phút.</p>
          
          <form className="space-y-3.5" onSubmit={(e) => { e.preventDefault(); alert('Đã đặt lịch thành công! Chúng tôi sẽ gọi xác nhận sớm nhất.'); setTestDriveOpen(false); }}>
            <Input required placeholder="Họ và tên" className="h-12 bg-white/80 border-[#4B0076]/10 focus-visible:ring-purple-400 rounded-xl" />
            ...
            <Input required type="date" className="h-12 bg-white/80 border-[#4B0076]/10 focus-visible:ring-purple-400 rounded-xl" />
            ...
            <Button type="submit" className="w-full bg-gradient-to-br from-[#4B0076] to-[#9366D9] text-white font-bold h-12 rounded-full shadow-[0_8px_32px_rgba(75,0,118,0.35)] hover:-translate-y-0.5 mt-2 transition-transform">
              Xác nhận đặt lịch
            </Button>
          </form>
        </DialogContent>
